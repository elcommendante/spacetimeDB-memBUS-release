...
## 2.10.0-R3 candidate — 2026-09-11

Supersedes 2.10.0-R2. Same three components and package layout; one new relay feature, rebuilt binaries, three new package test assertions. No benchmark was re-run: the R2 numbers, charts and `benchmark-report.md` stand unchanged.

- Added (SpacetimeDB-memBUS-relay, fork ChangeLog 0.0.0.114): channel access rule `access = "table"`. A channel with this rule also takes `membership_table = "<table name>"` (required there, rejected on any other rule) and admits an identity only while that module table holds a row naming the identity and exactly that channel. The table must have a column `identity` of type `Identity` and a column `channel` of type `String`; other columns are ignored. `access = "authenticated"` is unchanged.
- Added: `RelayErrorCode::NotAMember` (value 10, appended; codes 0–9 keep their wire values). Subscribing to, or publishing on, a table channel without a membership row is rejected with it.
- Added: revocation. When the membership row is deleted in a committed transaction — a changed `channel` value is a delete of the old pair — every subscription of that identity on that channel is removed exactly as if the client had unsubscribed: neighbours receive `RelayLeave` and the client receives `NotAMember` with `request_id = 0`. A row deleted and re-inserted unchanged in the same transaction is an update and revokes nothing.
- Validation, zero fallback: if the published module has no such table, or a required column is absent or mistyped, the server logs the exact reason on `membus.relay.admission` and rejects subscriptions to that channel with `ChannelDenied`. It never degrades to `authenticated`. Table ids are resolved once per database and re-resolved when the module hash changes (publish/update).
- Cost: one read transaction per subscribe (and per publish rejection); a subscribed member publishes without touching the datastore. A committed transaction is inspected only when a table channel is configured and it wrote a resolved membership table.
- Package: `config\relay.toml` (used by both the full endpoints and `relay-only`) gains the channel `zone/members` with the new rule; the demo module gains the public table `relay_membership` and the reducers `relay_grant(identity, channel)` / `relay_revoke(identity, channel)`. `Run-Relay-Test.ps1` grew from 17 to **20 CHECK assertions**: non-member rejected with `NotAMember`, granted member accepted and delivered to, revoked member unsubscribed with `NotAMember` while the neighbour receives `RelayLeave`. README, INSTRUCTION, `configuration.template.md`, `documentation\relay-channel.md` and `documentation\configuration.md` describe the rule.
- Clean-extraction acceptance (headless, `packaging\Test-CleanExtraction-2.10.0-R3.ps1`, log `packaging\acceptance-2.10.0-R3-20260911.log`): integrity PASS 46 files; Run-MemBus-Test 3/3; Run-Relay-Test 20/20 CHECK; Run-Ephemeral-Restart-Test PASS; memBUS 2/2 after restart; Run-Allocator-Restart-Test PASS (ids 2 → 2, 4098, 4099; flipped table 0 rows); relay-only 20/20 CHECK, restart PASS, allocator PASS; graceful stop; reset clean; no leftover state — RESULT PASS.
- Asset: `SpacetimeDB-memBUS-2.10.0-R3-Windows-x64.zip`, 151,936,236 bytes, SHA-256 `6a4100c67374e30e0fd8355bee2ccf6137dac571f02f9aa46539c1c4b40a5178`; extracted package 46 files / 357,444,103 bytes; product standalone `2d9fed9dc8a672da95442931ebfca9efc641c758066ba7e70f47a6ad88899b0b`, relay-only standalone `3863c66f6acb8c9d3098b434df5a54010aaf5c90ee3cd6125f019126b169f1c4`, reference relay client `d6aecb27f3178382292069dcfcc9502116899c2697441d1b9b3205212ab9e745`, demo module `065a36800ab3947ff3477018cbb15761219fe19a42e6c586ec854718f8219083` and flip build `ab015ef5be7e26e60762c624d7c877e4695baf4e2a0161fb51c742f49e734253`. Kept in ignored `dist`, not committed.
- Open, unchanged from R2: project-lead extracted-package test, six-hour soak, RELEASE-tier benchmark repetition on a dedicated host, memBUS transport campaign on 2.10.0.

## 2.10.0-R2 candidate — 2026-09-10

Supersedes 2.10.0-R1 (never to be deployed: its binaries carry the two defects below). Same three components, same package layout, fixed binaries, one new test script, re-measured benchmarks.

- Fixed (upstream SpacetimeDB 2.10.0 behaviour, fork ChangeLog 0.0.0.113): a reducer that allocated an `auto_inc` id and then failed left the in-memory sequence cursor advanced while the persisted cursor stayed behind; the next commit handed out an id that was never persisted as allocated and, after a restart, the same id was issued again (`UniqueConstraintViolation` on the primary key). Rollback now resets the allocation cursor so the next transaction re-allocates and persists it. Values handed out by an aborted transaction are still not reused (upstream contract).
- Fixed (SpacetimeDB-memBUS-ephemeral): the automatic durable → ephemeral migration did not persist the clear of the old rows, so replay brought them back after a restart. The table now joins the ephemeral set at commit, after the flipping transaction's durable record is written, and replay clears every registered ephemeral user table (heals data roots written by R1).
- Package: new `Run-Allocator-Restart-Test.ps1` (alpha and relay-only) — publishes the demo module into a disposable probe database, aborting insert, committing insert, flips `flip_marker` to ephemeral by publishing `modules\db_membus_relay_e2e_flip.wasm`, `Stop-Process -Force`, restart, two inserts; passes when the new ids are unique and above the committed one and the flipped table is empty. Demo module gains `allocation_probe`, `allocation_insert`, `flip_marker`, `set_flip_marker`.
- Fixed (send path, upstream file `adaptive_recv.rs`, R1 findings R02-C1 / R03-200): the subscription send worker slept on a Tokio timer during its linger window without listening to its channel, so a reply enqueued in that window waited 1–16 ms on Windows. Idle ephemeral reducer call 15.9 ms → 0.058 ms; 200-client ephemeral P99 101.7 → 8.7 ms; durable rows unchanged.
- Benchmarks re-run on the R2 binaries (2026-09-10, QUICK tier): relay 200 clients × 20 Hz with AOI 58,764 deliveries/s at 1.03 / 1.97 ms P50 / P99 on 0.9 core and 0 B commit log; reducers + subscriptions 200 clients: ephemeral 2.95 / 8.71 ms (0 B), durable 8.71 / 17.7 ms (24.74 MB per 30 s); closed-loop ephemeral write 0.058 ms P50 at 16,081 tx/s on one client. Cross-database calls re-measured on 2.10.0 for the first time: full call with destination commit 0.58 / 0.81 ms P50 / P99 memBUS vs 1.30 / 2.06 ms HTTP from a procedure, 108 k vs 89 k ops/s at 32 × 8. Chart PNG/HTML, README tables, `benchmarks.md`, `ephemeral-tables.md` and the package `benchmark-report.md` updated; the 2.6.1 R6 transport table is kept as history only.
- Clean-extraction acceptance (headless, `packaging\Test-CleanExtraction-2.10.0-R2.ps1`, log `packagingcceptance-2.10.0-R2-20260910.log`): integrity PASS; Run-MemBus-Test 3/3; Run-Relay-Test 17/17 CHECK; Run-Ephemeral-Restart-Test PASS; memBUS 2/2 after restart; **Run-Allocator-Restart-Test PASS** (ids 2 → 2, 4098, 4099; flipped table 0 rows); relay-only 17/17, restart PASS, allocator PASS; graceful stop; reset clean — RESULT PASS.
- Asset: `SpacetimeDB-memBUS-2.10.0-R2-Windows-x64.zip`, 152,012,294 bytes, SHA-256 `0b7fe4060c9e2486834839259b56fdde8adc6435df5068259b56f71836f32114`; extracted package 46 files / 357,625,757 bytes; product standalone `4a248731839e3d3b69a1cb84dc138fa8711cab3ae10fc7430a429945a200e2da`, relay-only standalone `38e1b84cb2cf0505f377d7459a3184b2bb58bdd4f4b032cfbf9565a58a845827`. Kept in ignored `dist`, not committed.

## 2.10.0-R1 candidate — 2026-09-09 (superseded by R2, do not deploy)
