...
-> generate a session JWT key pair under run\jwt-keys
-> start alpha and beta inside the handshake window
-> wait for both listeners and route readiness
-> run/inspect memBUS calls, relay/ephemeral smoke, restart contract
-> Ctrl+C relay-only, beta, alpha
...
Capability files and the JWT key pair are generated locally under `run`; no package secret exists. 2.10.0 requires the key pair as explicit paths, so `Reset-Demo.ps1` deletes it and every session gets a new one (old anonymous tokens become invalid by design). Do not copy runtime capability/JWT material into bug reports. Logs must not include payload bodies or player data by default.
